"""Build Stereo Upmix 2 to 7.0 with four analysis bands for PureData.

``STEM`` identifies faust/dsp/faustgen-upmix-surround-7ch-multiband.dsp and the exported files.
...
Usage: ``python generate_faustgen_upmix_surround_7ch_multiband.py [--output-dir DIRECTORY] [--faust EXECUTABLE]
...
STEM = "faustgen-upmix-surround-7ch-multiband"
...
def build_patch(project):
    """Build the PureData patch for Stereo Upmix 2 to 7.0 with four analysis bands in memory.
