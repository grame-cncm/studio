"""Build mono-to-stereo spatialization with Zita Rev1 for PureData.

``STEM`` identifies faust/dsp/faustgen-mono-stereo-spatial-reverb.dsp and the exported files.
...
Usage: ``python generate_faustgen_mono_stereo_reverb.py [--output-dir DIRECTORY] [--faust EXECUTABLE]
...
STEM = "faustgen-mono-stereo-spatial-reverb"


def build_patch(project):
    """Build the PureData patch for mono-to-stereo spatialization with Zita Rev1 in memory.
    ...
    Real mono input or 220 Hz sine → panning and Zita → two outputs.
