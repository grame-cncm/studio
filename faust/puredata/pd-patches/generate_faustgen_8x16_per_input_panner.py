"""Build independent VBAP for eight inputs and sixteen speakers for PureData.

``STEM`` identifies faust/dsp/faustgen-8x16-per-input-panner.dsp and the exported files.
...
Usage: ``python generate_faustgen_8x16_per_input_panner.py [--output-dir DIRECTORY] [--faust EXECUTABLE]
...
STEM = "faustgen-8x16-per-input-panner"


def build_patch(project):
    """Build the PureData patch for independent VBAP for eight inputs and sixteen speakers in memory.
    ...
    Eight hardware inputs → independent VBAP positions → sixteen outputs.
