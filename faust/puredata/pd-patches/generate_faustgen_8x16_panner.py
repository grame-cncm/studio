"""Build global panning, eight inputs to sixteen speakers for PureData.

``STEM`` identifies faust/dsp/faustgen-8x16-panner.dsp and the exported files.
...
Usage: ``python generate_faustgen_8x16_panner.py [--output-dir DIRECTORY] [--faust EXECUTABLE]
...
STEM = "faustgen-8x16-panner"


def build_patch(project):
    """Build the PureData patch for global panning, eight inputs to sixteen speakers in memory.
    ...
    Eight hardware inputs → shared panning → sixteen outputs.
