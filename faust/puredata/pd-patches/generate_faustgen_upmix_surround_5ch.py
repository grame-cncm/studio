"""Build Stereo Upmix 2 to 5.0, center and decorrelated surrounds for PureData.

``STEM`` identifies faust/dsp/faustgen-upmix-surround-5ch.dsp and the exported files.
...
Usage: ``python generate_faustgen_upmix_surround_5ch.py [--output-dir DIRECTORY] [--faust EXECUTABLE]
...
STEM = "faustgen-upmix-surround-5ch"

# Faust outputs FL, FR, C, Ls, Rs, in order, to the studio's M layer (1.24 m high,
...
def build_patch(project):
    """Build the PureData patch for Stereo Upmix 2 to 5.0, center and decorrelated surrounds in memory.
    ...
    independent noises)
    → upmix → FL M1 (11), FR M2 (12), C AtmoC (28), Ls M3 (13), Rs M4 (14). Faust audio outlet c+1 reaches dac~ inlet c, played on the c-th listed output.
